...
// New function to get token for 3x-ui (supports both old and new API)
function getXuiToken($server_id){
    global $connection;
    ...
    // Try new API first (3x-ui new version)
    $loginUrl = $panel_url . '/api/admin/token';
    $postFields = array(
        'username' => $username,
        'password' => $password
    ...
    // If new API works (returns token), use it
    if ($httpCode == 200) {
        $tokenResponse = json_decode($response);
        if (isset($tokenResponse->access_token)) {
            return (object) ['api_version' => 'new', 'token' => $tokenResponse];
        }
    }
    
    // Fallback to old API (cookie-based)
    return (object) ['api_version' => 'old', 'token' => null];
}
...
function getJson($server_id){
    ...
    if ($tokenInfo->api_version == 'new' && isset($tokenInfo->token->access_token)) {
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                'Accept: application/json',
                'Authorization: Bearer ' . $tokenInfo->token->access_token
            ),
            CURLOPT_SSL_VERIFYHOST => false,
            CURLOPT_SSL_VERIFYPEER => false,
        ));
        ...
        if ($httpCode == 200) {
            ...
            if (isset($result->obj) || isset($result->success)) {
                return $result;
            }
            // If response has direct array structure, wrap it
            if (is_array($result)) {
                return (object) ['success' => true, 'obj' => $result];
            }
        }
    }
