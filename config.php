...
/**
 * Convert old Sanaei API endpoint to new v2.8.5+ format
 * Automatically handles backward compatibility with v1.7.9
 */
function getSanaeiEndpoint($endpoint, $serverType) {
    // Only convert for Sanaei panels
    if ($serverType !== "sanaei") return $endpoint;
    
    // Convert old format to new API format
    // v1.7.9: /panel/inbound/* -> v2.8.5: /panel/api/inbounds/*
    $newEndpoint = str_replace('/panel/inbound/', '/panel/api/inbounds/', $endpoint);
    
    return $newEndpoint;
}

/**
 * Check if response indicates endpoint not found (404 or similar error)
 */
function isEndpointNotFound($response) {
    ...
    if ($responseObj && isset($responseObj->success) && $responseObj->success === false) {
        // Check if error message indicates not found
        if (isset($responseObj->msg) && (
            stripos($responseObj->msg, 'not found') !== false ||
            stripos($responseObj->msg, '404') !== false ||
            stripos($responseObj->msg, 'invalid') !== false
        )) {
            return true;
        }
    }
...
function deleteClient($server_id, $inbound_id, $uuid, $delete = 0){
    ...
    if($delete == 1){
        ...
        if($serverType == "sanaei" || $serverType == "alireza"){
            // For Sanaei, try new v2.8.5 API first, fallback to v1.7.9
            if($serverType == "sanaei") {
                $endpoint = "/panel/inbound/" . $inbound_id . "/delClient/" . rawurlencode($uuid);
                $url = $panel_url . getSanaeiEndpoint($endpoint, $serverType);
            }
            ...
            // If new endpoint failed for Sanaei, try old endpoint
            if($serverType == "sanaei" && isEndpointNotFound($response)){
                $old_url = "$panel_url/panel/inbound/" . $inbound_id . "/delClient/" . rawurlencode($uuid);
                curl_setopt($curl, CURLOPT_URL, $old_url);
                $response = curl_exec($curl);
            }
        }else{
            curl_setopt_array($curl, array(
